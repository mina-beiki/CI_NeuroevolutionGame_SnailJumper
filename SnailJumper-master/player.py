...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            # 32: calculated 32 by using test and error.
            # 2: output size is 2 like before
            # 8: number of features
            # the features I considered:
            # I considered the 3 upper obstacles and each of them have x and y so till here we have 6 features
            # I considered the x and y of the snail, so 2 features are added so we overall we have 8 features
            layer_sizes = [8, 32, 2]  # TODO (Design your architecture here by changing the values)
            self.nn = NeuralNetwork(layer_sizes)

    def generate_nn_input(self, screen_height, obstacles, player_x, player_y):

        nn_input = [player_x, player_y]
        #for the obstacles only the 3 upper obstacles matter, in the screen y of the up is 0 and
        #y of the down is max so we compare y-2 to player's y and if it is smaller then we add
        #it to the input and if not, we don't consider it.(we consider it too far and add 0,0 )
        for i in range(3):
            try:
        ...
        #batch normalization: (ranges from -1 to 1)
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        nn_input = self.generate_nn_input(screen_height, obstacles, player_x, player_y)
